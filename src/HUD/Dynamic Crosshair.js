...
function getLookVector(player) {
    ...
    // Fallback: compute manually from pitch/yaw
...
function detectTarget(game, player) {
    ...
    // Throttle raycast to every 100ms (still responsive but not heavy)
    if (now - state.lastRaycastScan < 100) {
        return state.currentSituation;
    }
    state.lastRaycastScan = now;

    // Check if airborne (isAirborne is unreliable, use onGround + motion.y)
    ...
    // Check block targeting via selectBox
    const targetingBlock = player.selectBox?.visible === true;
    ...
    // Determine situation by priority
    if (closestEntity) {
        // It's an entity — classify it
        if (closestEntity.profile) {
            // It's a player
            return setSituation('player');
        }

        if (closestEntity.creatureClass) {
            // Hostile mob
            return setSituation('enemy');
        }

        // Other entity (item, projectile, etc.)
        const name = String(closestEntity.name || '').toLowerCase();
        if (name.includes('item') || name.includes('drop')) {
            return setSituation('item');
        }

        if (name.includes('arrow') || name.includes('snowball') ||
            name.includes('egg') || name.includes('pearl')) {
            return setSituation('projectile');
        }

        return setSituation('entity');
    }

    // Check movement-based situations
    const motion = player.motion;
    if (motion) {
        const speed = Math.hypot(Number(motion.x) || 0, Number(motion.z) || 0);
        const vertSpeed = Number(motion.y) || 0;

        // Bridging: sneaking and moving slowly while looking down
        if (player.sneak && speed > 0.01 && (Number(player.pitch) || 0) > 0.5) {
            return setSituation('bridging');
        }

        // Building: punching/using on a block
...
function getCrosshairUrl(filename) {
    ...
    try {
        ...
        // Fallback: direct extension path (works in MAIN world in some setups)
        return `chrome-extension://${chrome.runtime.id}/assets/crosshair/${file}`;
    }
}
...
function hideOriginalCrosshair() {
    if (!state.originalSvg) {
        ...
        state.originalSvgParent = state.originalSvg?.parentElement;
    }
...
function loop() {
    if (state.enabled) {
        ...
        if (game && player) {
            // Re-find crosshair if it was replaced by game (React re-renders)
